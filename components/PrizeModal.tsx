...
export default function PlayerPersonalHub() {
  ...
  // 🔥 NOVO: GAVETA LATERAL DO MINI-HUB 🔥
  const [showMiniHub, setShowMiniHub] = useState(false);
    ...
    <div className="min-h-[100dvh] bg-[#050505] text-white font-sans pb-24 relative overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full h-20 bg-[#0a0a0a]/80 backdrop-blur-2xl border-b border-white/5 z-[100] px-4 sm:px-6 flex items-center justify-between shadow-2xl">
          ...
          <div className="flex items-center gap-3 sm:gap-6">
              ...
              {/* 🔥 BOTÃO PARA ABRIR O MINI HUB DO CLIENTE (Substituindo o antigo Sair) 🔥 */}
              <button onClick={() => setShowMiniHub(true)} className="p-2 sm:px-4 sm:py-2 bg-[#D946EF]/20 border border-[#D946EF]/50 text-[#D946EF] rounded-full text-[9px] font-black uppercase shadow-xl hover:bg-[#D946EF] hover:text-white transition-all flex items-center gap-2">
                 <User size={14} /> <span className="hidden sm:block">Meu Perfil</span>
              </button>
          </div>
      </header>

      {/* 🔥 MINI HUB DO CLIENTE (Drawer Lateral) 🔥 */}
      {showMiniHub && (
          <>
              <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200]" onClick={() => setShowMiniHub(false)}></div>
              <div className="fixed top-0 right-0 h-full w-full sm:w-96 bg-[#0a0a0a] border-l border-white/10 z-[210] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
                  <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black/50 shrink-0">
                      <h2 className="text-lg font-black uppercase italic text-[#D946EF] flex items-center gap-2"><User size={20}/> Conta do Jogador</h2>
                      <button onClick={() => setShowMiniHub(false)} className="p-2 bg-white/5 rounded-full hover:bg-white/10 text-white/50 hover:text-white transition-all"><X size={18}/></button>
                  </div>
                  
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                      <div className="bg-black/50 border border-white/10 p-5 rounded-2xl text-center">
                          <p className="text-[10px] font-black uppercase text-white/50 tracking-widest mb-2">Telefone Vinculado</p>
                          <p className="text-lg font-black text-white">{playerPhone}</p>
                      </div>

                      <div>
                          <h3 className="text-xs font-black uppercase text-[#D946EF] mb-3 tracking-widest flex items-center gap-2"><Wallet size={14}/> Seus Saldos de Roleta</h3>
                          <div className="space-y-2">
                              {associations.map((assoc: any) => (
                                  <div key={assoc.id} className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
                                      <span className="text-[10px] font-black uppercase text-white truncate max-w-[150px]">{assoc.modelInfo?.model_name}</span>
                                      <span className="text-xs font-black text-[#D946EF]">{assoc.credits} CR</span>
                                  </div>
                              ))}
                              {associations.length === 0 && <p className="text-[10px] italic text-white/30 text-center py-2">Você não tem saldo em nenhuma musa.</p>}
                          </div>
                      </div>
                      
                      <button onClick={() => { localStorage.clear(); window.location.replace('/'); }} className="w-full py-4 mt-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl text-[10px] font-black uppercase shadow-xl hover:bg-red-500 hover:text-white transition-all">Sair da Conta</button>
                  </div>
              </div>
          </>
      )}

      <main className={`max-w-7xl mx-auto p-4 sm:p-8 mt-28 transition-all ${(chatOpen || showMiniHub) ? 'blur-sm brightness-50' : ''}`}>
        
        {/* CORPO DO PAINEL DO JOGADOR... (Seção de Chat, Videos, Coleção) */}
      ...
      {/* LOJA DE LIVETOKENS E MODAL PIX */}
      {showShopModal && (
