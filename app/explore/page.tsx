...
import { Wallet, Search, Flame, Lock, Coins, Sparkles, Loader2, User, Home, Heart, Maximize2, X, QrCode, Copy, CheckCircle, Bell, LogOut, Radio } from "lucide-react";
...
export default function ExploreLivePage() {
      ...
      try {
        ...
        if (mRes.ok && cRes.ok) {
            ...
            const combinedData = mData.map((m: any) => {
                ...
                return {
                    id: m.id, name: config?.model_name || m.slug, slug: m.slug,
                    image: config?.profile_url || null, bio: config?.bio || "Acompanhe minhas lives e conteúdos quentes!",
                    status: m.live_status || "offline", price: config?.live_price || 3.10, 
  ...
  const handleJoinLive = (slug: string, status: string) => { requireAuth(() => router.push(`/live/${slug}`)); };
  const openMiniProfile = (e: React.MouseEvent, model: any) => { e.stopPropagation(); setSelectedModel(model); setIsFollowing(false); };
  
  const handleLogout = () => {
    if(confirm("Deseja mesmo sair?")) {
        localStorage.removeItem("labz_player_phone"); localStorage.removeItem("labz_player_logged");
        setPlayerPhone(null); setBalance(0); router.refresh();
    }
  };
  ...
  const generatePix = async (amount: number) => {
    requireAuth(async () => {
        ...
        try {
            ...
            const playerId = (await pRes.json())[0]?.id;
            ...
            if (data.qr_code_base64 || data.qrCodeBase64) {
                setPixData({ qrCodeBase64: data.qr_code_base64 || data.qrCodeBase64, qrCodeCopiaCola: data.qr_code || data.qrCode || data.copy_paste, value: amount });
                setInitialBalanceCheck(balance); setShowShopModal(false); setShowPixModal(true);
            }
      ...
      if (showPixModal && pixData && !paymentSuccess && playerPhone) {
          interval = setInterval(async () => {
              try {
                  ...
                  const currentTokens = (await res.json())[0]?.live_tokens || 0;
                  if (currentTokens > initialBalanceCheck) {
                      setBalance(currentTokens); clearInterval(interval); setPaymentSuccess(true);
  ...
  if (initialLoading) return <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white"><Loader2 className="animate-spin text-[#00f0ff] mb-6" size={50} /><h2 className="text-xl font-black uppercase italic animate-pulse tracking-widest">Carregando Musas...</h2></div>;
    ...
    <div className="min-h-[100dvh] bg-[#050505] text-white pb-20 font-sans relative">
      ...
      {showAuthModal && (
          <div className="fixed inset-0 z-[500] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
              <div className="bg-[#0a0a0a] border border-[#00f0ff]/30 p-8 rounded-[2rem] w-full max-w-sm shadow-2xl relative text-center">
                  ...
                  <div className="w-16 h-16 bg-[#00f0ff]/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-[#00f0ff]/30"><Lock size={28} className="text-[#00f0ff]"/></div>
                  ...
                  <button onClick={() => router.push('/')} className="w-full bg-[#00f0ff] text-black py-4 rounded-xl text-[11px] font-black uppercase shadow-[0_0_20px_rgba(0,240,255,0.4)]">Fazer Login ou Cadastro</button>
              </div>
          </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex flex-col items-center justify-center p-6 text-center animate-fadeIn">
          ...
          <div className="flex flex-col gap-4 w-full max-w-sm">
            {[ {name: "Básico", p: 30}, {name: "VIP", p: 50}, {name: "Premium", p: 100} ].map(pkg => (
              <button key={pkg.p} onClick={() => generatePix(pkg.p)} disabled={generatingPix} className="flex items-center justify-between bg-white/5 border border-white/10 p-4 rounded-2xl hover:border-[#00f0ff]/50">
                <span className="text-white font-black uppercase text-sm">{pkg.name}</span><span className="bg-[#00f0ff] text-black px-4 py-1.5 rounded-full font-black text-xs">{generatingPix ? <Loader2 size={12} className="animate-spin inline" /> : `${pkg.p} LT`}</span>
              </button>
            ))}
          </div>
        </div>
              ...
              <div className="bg-[#0a0a0a] border border-[#00f0ff]/30 p-8 sm:p-10 rounded-[3rem] w-full max-w-md shadow-2xl relative text-center">
                  ...
                  {paymentSuccess ? (
                      ...
                      <>
                          ...
                          <div className="bg-white p-4 rounded-[2rem] mx-auto w-48 h-48 mb-6 flex items-center justify-center"><img src={pixData.qrCodeBase64.includes('data:image') ? pixData.qrCodeBase64 : `data:image/png;base64,${pixData.qrCodeBase64}`} className="w-full h-full object-contain rounded-xl" /></div>
                          <p className="text-3xl font-black text-white mb-6">R$ {pixData.value.toFixed(2)}</p>
                          <button onClick={() => { navigator.clipboard.writeText(pixData.qrCodeCopiaCola); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 text-white py-5 rounded-2xl font-black uppercase text-xs mb-4">{copied ? <CheckCircle size={18} className="text-[#00f0ff]" /> : <Copy size={18} />} {copied ? "Copiado!" : "Copiar Chave PIX"}</button>
                      </>
                  )}
              </div>
          </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center sm:p-4">
            ...
            <div className="relative w-full max-w-lg bg-[#0a0a0a] border border-white/10 sm:rounded-[2.5rem] rounded-t-[2.5rem] flex flex-col shadow-2xl overflow-hidden z-10 animate-slideUp">
                <div className="h-48 relative bg-black border-b border-white/10">
                    ...
                    <div className="absolute -bottom-10 left-6 w-24 h-24 rounded-full border-4 border-[#0a0a0a] overflow-hidden bg-black shadow-xl"><img src={selectedModel.image} className="w-full h-full object-cover" /></div>
                </div>
                <div className="pt-12 px-6 pb-6 flex flex-col">
                    ...
                    <p className="text-sm text-white/70 italic leading-relaxed mb-6 bg-white/5 p-4 rounded-2xl border border-white/5 line-clamp-3">"{selectedModel.bio}"</p>
                    ...
                    <div className="flex gap-3 overflow-x-auto custom-scrollbar pb-2 mb-6">
                        {selectedModel.publicPhotos.map((photo: string, idx: number) => (<div key={idx} className="w-24 h-32 shrink-0 rounded-xl overflow-hidden border border-white/10"><img src={photo} className="w-full h-full object-cover" /></div>))}
                        {selectedModel.publicPhotos.length === 0 && <p className="text-xs text-white/30 italic font-bold">Nenhuma foto pública gratuita.</p>}
                    </div>
                    <div className="grid grid-cols-2 gap-3 mt-auto">
                        <button onClick={() => requireAuth(() => router.push(`/profile/${selectedModel.slug}`))} className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white py-4 rounded-xl text-[10px] font-black uppercase border border-white/10"><Maximize2 size={14}/> Perfil Completo</button>
                        <button onClick={() => handleJoinLive(selectedModel.slug, selectedModel.status)} className="flex items-center justify-center gap-2 bg-[#00f0ff] text-black py-4 rounded-xl text-[10px] font-black uppercase shadow-[0_0_20px_rgba(0,240,255,0.4)]"><Sparkles size={14}/> Entrar na Live</button>
                    </div>
                </div>
            </div>
        </div>
      ...
      <header className="sticky top-0 z-40 bg-black/60 backdrop-blur-xl border-b border-white/10 px-4 py-4 flex items-center justify-between shadow-2xl">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-black uppercase tracking-widest bg-clip-text text-transparent bg-gradient-to-r from-white to-white/50 hidden sm:block">Labz <span className="text-[#00f0ff]">Live</span></h1>
        </div>
        <div className="flex items-center gap-3">
          {/* 🔥 AQUI MUDEI O NOME DO BOTÃO PARA VITRINE 🔥 */}
          <button onClick={() => requireAuth(() => router.push('/vitrine'))} className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-full transition-all">
            ...
            <span className="font-black text-[9px] uppercase text-white/70 tracking-widest hidden sm:block">Vitrine</span>
          </button>
          ...
          {playerPhone && <button onClick={handleLogout} className="flex items-center justify-center w-10 h-10 bg-red-500/10 text-red-500 border border-red-500/20 rounded-full"><LogOut size={16} /></button>}
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6 space-y-8">
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar musas ao vivo..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-sm font-medium text-white outline-none focus:border-[#00f0ff]" />
        </div>

        {trendingModel && (
        <section>
          <div className="flex items-center gap-2 mb-4"><Flame className="text-[#ff0055]" size={18} /><h2 className="text-sm font-black uppercase tracking-widest text-white/80">Em Alta Agora</h2></div>
          <div onClick={(e) => openMiniProfile(e, trendingModel)} className="h-48 rounded-[2rem] bg-[#0a0a0a] border border-white/10 overflow-hidden relative flex items-center justify-between cursor-pointer hover:border-[#00f0ff]/50 transition-colors shadow-2xl group p-6 sm:p-8">
             <div className="absolute inset-0"><img src={trendingModel.image} className="w-full h-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-700" /><div className="absolute inset-0 bg-gradient-to-r from-black/90 via-black/50 to-transparent"></div></div>
             <div className="relative z-10 flex flex-col items-start max-w-[70%]">
               <span className="bg-[#ff0055] text-white text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full mb-2 shadow-[0_0_15px_rgba(255,0,85,0.5)]">{trendingModel.status === 'vip' ? 'Em Show VIP' : 'Destaque'}</span>
        ...
        <section>
          <div className="flex items-center justify-between mb-4"><h2 className="text-sm font-black uppercase tracking-widest text-white/80">Câmeras</h2><span className="text-[10px] text-white/40 font-bold uppercase">{filteredModels.length} Musas</span></div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredModels.map((model) => (
              <div key={model.id} onClick={() => handleJoinLive(model.slug, model.status)} className="group relative aspect-[3/4] rounded-3xl overflow-hidden cursor-pointer bg-[#0a0a0a] border border-white/10 hover:border-[#00f0ff]/50 shadow-xl">
                {model.image ? <img src={model.image} className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${model.status === 'offline' ? 'grayscale opacity-30' : 'opacity-80 group-hover:opacity-100'}`} /> : <div className="w-full h-full flex items-center justify-center bg-white/5"><User size={40} className="text-white/20" /></div>}
                ...
                <div className="absolute top-3 left-3 z-10">
                  {model.status === 'online' && <span className="bg-black/60 backdrop-blur-md border border-[#00f0ff]/50 text-[#00f0ff] text-[9px] font-black uppercase px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-[0_0_10px_rgba(0,240,255,0.3)]"><span className="w-1.5 h-1.5 rounded-full bg-[#00f0ff] animate-pulse"></span> Ao Vivo</span>}
                  {model.status === 'vip' && <span className="bg-black/60 backdrop-blur-md border border-[#ff0055]/50 text-[#ff0055] text-[9px] font-black uppercase px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-[0_0_10px_rgba(255,0,85,0.3)]"><Lock size={10} /> Privado VIP</span>}
                  {model.status === 'offline' && <span className="bg-black/60 backdrop-blur-md border border-white/20 text-white/50 text-[9px] font-black uppercase px-3 py-1.5 rounded-full">Offline</span>}
                </div>

                {/* 🔥 O BOTÃO AGORA TEM TEXTO: "VER MINI PERFIL" 🔥 */}
                <div className="absolute top-3 right-3 z-20">
                  <button onClick={(e) => openMiniProfile(e, model)} className="flex items-center gap-1.5 bg-black/60 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white px-3 py-1.5 rounded-full transition-all shadow-lg">
                    <User size={12} />
                    <span className="text-[8px] font-black uppercase tracking-widest hidden sm:block">Mini Perfil</span>
                  </button>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 flex flex-col items-start transform translate-y-2 group-hover:translate-y-0 transition-transform z-10">
                  ...
                  <div className="flex items-center gap-1.5 mt-1 opacity-0 group-hover:opacity-100 transition-opacity delay-100"><Coins size={12} className="text-[#00f0ff]" /><span className="text-[#00f0ff] font-bold text-[10px]">{model.price.toFixed(2).replace('.', ',')} LT/min VIP</span></div>
                </div>
              </div>
            ))}
          </div>
