    ...
    <div className="w-full h-full flex items-center justify-center bg-black relative">
      {remoteTrack ? (
        ...
        <div className="flex flex-col items-center gap-4 text-[#D946EF]/50 z-20">
          ...
          <span className="font-black uppercase tracking-widest text-xs animate-pulse text-center px-6">Aguardando a modelo iniciar...</span>
        </div>
      )}
    </div>
  );
...
function FloatingActions({ clientName }: { clientName: string }) {
  ...
  const handleRequestPrivate = async () => {
    ...
    try {
      ...
      setTimeout(() => setRequestingPrivate(false), 15000); 
    }
  };
  ...
  return (
    <div className="absolute bottom-6 right-6 z-30 flex items-center gap-3 bg-black/60 backdrop-blur-lg p-2 rounded-full border border-white/10 shadow-2xl">
      <button onClick={() => alert("Janela de Presentes em breve!")} className="w-14 h-14 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-white transition-all group border border-white/5">
  ...
  return (
    <div className="h-screen w-full bg-[#050505] flex flex-col overflow-hidden">
      <header className="h-16 bg-[#0a0a0a] border-b border-white/5 flex items-center justify-between px-4 sm:px-6 shrink-0 z-50">
        ...
        <div className="flex items-center gap-2.5 bg-white/5 border border-white/10 px-4 py-2 rounded-full cursor-pointer hover:bg-white/10">
