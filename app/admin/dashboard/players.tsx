  ...
  async function fetchPlayers() {
    ...
    try {
      const res = await fetch(`${supabaseUrl}/rest/v1/Players?model_id=eq.${modelId}&select=*`, {
        method: "GET", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, cache: "no-store",
      });
      const data = await res.json();
      if (Array.isArray(data)) setPlayers(data);
    } catch (error) { setPlayers([]); } finally { setLoading(false); }
  }
  ...
  const resetCredits = async (playerName: string) => {
    ...
    try {
      const res = await fetch(`${supabaseUrl}/rest/v1/Players?name=eq.${playerName}&model_id=eq.${modelId}`, {
        method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ credits: 0 }),
      });
      if (res.ok) setPlayers((prev) => prev.map((p) => p.name === playerName ? { ...p, credits: 0 } : p));
  ...
  const addCreditsWithRevenue = async (playerName: string, currentCredits: number, realAmount: number, creditsToAdd: number) => {
    ...
    try {
      ...
      await fetch(`${supabaseUrl}/rest/v1/Players?name=eq.${playerName}&model_id=eq.${modelId}`, {
        method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ credits: newTotal }),
      });

      let modelShare = 0;
      if (realAmount > 0) {
        modelShare = realAmount * 0.70;
        const platformShare = realAmount * 0.30;
        
        const resModel = await fetch(`${supabaseUrl}/rest/v1/Models?id=eq.${modelId}&select=balance`, { headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}` } });
      ...
      if (modelShare > 0) alert(`Sucesso! ${creditsToAdd} CR adicionados.\nR$ ${modelShare.toFixed(2)} (70%) foram creditados no saldo da modelo.`);
    } catch (err) { alert("Erro ao processar transação."); } finally { setUpdatingName(null); }
  };
    ...
    <div className="animate-in fade-in duration-500 text-white">
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <input placeholder="Buscar jogador desta roleta..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 pl-12 pr-6 text-sm text-white outline-none focus:border-[#FF1493] transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <button onClick={() => setIsModalOpen(true)} className="bg-[#FF1493] hover:opacity-90 text-white font-black uppercase text-[10px] px-8 py-4 md:py-0 rounded-2xl flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(255,20,147,0.4)] transition-all active:scale-95">
            ...
            <div key={p.name || index} className="bg-[#121212] border border-white/5 p-6 rounded-[2.5rem] flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shadow-xl hover:border-white/10 transition-all">
              <div className="flex items-start justify-between w-full lg:w-auto lg:flex-1">
                <div className="flex items-center gap-5">
                  <div className="h-14 w-14 rounded-2xl bg-[#FF1493]/10 flex items-center justify-center text-[#FF1493] shrink-0 border border-[#FF1493]/20"><User size={24} /></div>
                ...
                <button onClick={() => deletePlayer(p.name)} disabled={updatingName === p.name} className="text-white/20 hover:text-red-500 transition-colors p-2 flex flex-col items-center shrink-0 ml-4"><UserMinus size={18} /><span className="text-[7px] font-bold uppercase mt-1">Excluir</span></button>
              </div>
              ...
              <div className="flex flex-col items-end gap-4 w-full lg:w-auto border-t lg:border-t-0 border-white/5 pt-4 lg:pt-0">
                <div className="flex items-center gap-4 w-full justify-between lg:justify-end">
                  ...
                  <button onClick={() => resetCredits(p.name)} disabled={updatingName === p.name} className="p-3 bg-red-500/10 text-red-500 rounded-2xl border border-red-500/20 active:scale-90" title="Zerar Créditos"><RotateCcw size={18} /></button>
                </div>
                ...
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <div className="flex flex-wrap justify-end gap-2">
                    <button onClick={() => addCreditsWithRevenue(p.name, p.credits, 10, 15)} disabled={updatingName === p.name} className="bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-black py-2 px-3 rounded-xl border border-emerald-500/30 transition-all active:scale-95 text-center"><span className="block text-[10px] font-black uppercase">R$ 10</span><span className="block text-[8px] font-bold">+15 CR</span></button>
                    <button onClick={() => addCreditsWithRevenue(p.name, p.credits, 20, 25)} disabled={updatingName === p.name} className="bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-black py-2 px-3 rounded-xl border border-emerald-500/30 transition-all active:scale-95 text-center"><span className="block text-[10px] font-black uppercase">R$ 20</span><span className="block text-[8px] font-bold">+25 CR</span></button>
                    <button onClick={() => addCreditsWithRevenue(p.name, p.credits, 50, 60)} disabled={updatingName === p.name} className="bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-black py-2 px-3 rounded-xl border border-emerald-500/30 transition-all active:scale-95 text-center"><span className="block text-[10px] font-black uppercase">R$ 50</span><span className="block text-[8px] font-bold">+60 CR</span></button>
                    <div className="flex items-center bg-white/5 border border-white/10 rounded-xl px-2">
                      <div className="flex items-center border-r border-white/10 pr-2"><DollarSign size={12} className="text-white/30 mr-1"/><input type="number" placeholder="R$ Pago" value={manualReal[p.name] || ""} onChange={(e) => setManualReal({ ...manualReal, [p.name]: e.target.value })} className="bg-transparent w-12 text-center text-[10px] outline-none text-emerald-400 font-bold placeholder:text-white/20"/></div>
                      <div className="flex items-center pl-2"><Wallet size={12} className="text-white/30 mr-1"/><input type="number" placeholder="CR" value={manualCredit[p.name] || ""} onChange={(e) => setManualCredit({ ...manualCredit, [p.name]: e.target.value })} className="bg-transparent w-10 text-center text-[10px] outline-none text-[#FF1493] font-bold placeholder:text-white/20"/></div>
                      <button onClick={() => addCreditsWithRevenue(p.name, p.credits, Number(manualReal[p.name] || 0), Number(manualCredit[p.name] || 0))} disabled={updatingName === p.name || !manualCredit[p.name]} className="text-emerald-400 hover:text-white p-2 ml-1 transition-colors"><Check size={16} /></button>
                    </div>
                  </div>
                </div>
              </div>
