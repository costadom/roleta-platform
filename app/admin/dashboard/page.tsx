...
import { 
  ...
  HelpCircle, ChevronUp, ChevronDown, User, Globe, Camera, Video, Send, Trash2, LayoutGrid, CheckCircle, Clock, AlertTriangle, Settings, Eye, EyeOff, X, Upload
} from "lucide-react";
...
function DashboardContent() {
  ...
  const [activeTab, setActiveTab] = useState<"finance" | "hub" | "gallery" | "video_requests" | "roleta" | "players" | "history">("finance");
  ...
  const [rawPrice, setRawPrice] = useState(""); // Valor centavos
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [resMod, resTrans, resPrizes, resConfig, resMedia, resVideos, resHistory] = await Promise.all([
  ...
  const onPublishPhoto = async () => {
    ...
    if (res.ok) {
        ...
        setNewMediaCaption(""); setRawPrice(""); setIsPaidMedia(false); setSelectedGalleryFile(null); setGalleryPreviewUrl(null); loadData(); alert("Publicada!");
    }
    setUploading(false);
  };
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-4 sm:p-8 font-sans pb-24">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <button onClick={() => isSuper ? router.push('/admin/super') : (localStorage.clear(), router.push('/admin'))} className="flex items-center gap-2 text-[10px] font-black uppercase text-white/30 bg-white/5 px-4 py-2 rounded-xl">
        ...
        <div className="flex gap-2 mb-8 bg-white/5 p-1.5 rounded-2xl border border-white/5 overflow-x-auto custom-scrollbar">
          <button onClick={() => setActiveTab("finance")} className={`flex-1 min-w-[100px] py-3 rounded-xl text-[9px] font-black uppercase transition-all ${activeTab === "finance" ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30 border shadow-lg" : "text-white/30 hover:bg-white/5"}`}>Ganhos</button>
            ...
            <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-black border border-emerald-500/30 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden text-center">
                        <h2 className="text-xs font-black uppercase mb-2 text-emerald-500">Saldo Disponível (70%)</h2>
                    ...
                    <div className="bg-black border border-[#FFD700]/30 p-8 rounded-[2.5rem] shadow-2xl flex flex-col justify-center text-center">
                        <h2 className="text-xs font-black uppercase mb-2 text-[#FFD700]">Total Acumulado</h2>
                ...
                <div className="bg-black border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
                    <h2 className="text-xs font-black uppercase mb-4 text-[#FF1493] flex items-center gap-2"><DollarSign size={16}/> Configurar Chaves PIX</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <input type="text" value={pixKey1} onChange={e => setPixKey1(e.target.value)} className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-xs text-white" placeholder="Chave PIX Principal" />
                        <input type="text" value={pixKey2} onChange={e => setPixKey2(e.target.value)} className="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-xs text-white" placeholder="Chave PIX Secundária" />
                    </div>
        ...
        {activeTab === "hub" && (
            <div className="max-w-3xl mx-auto bg-black border border-white/10 p-10 rounded-[3rem] shadow-2xl animate-in slide-in-from-bottom-4">
                <h2 className="text-xl font-black uppercase italic mb-8 text-[#FF1493]">Editar Bio do Hub</h2>
                <textarea value={bio} onChange={e => setBio(e.target.value)} className="w-full bg-black border border-white/10 rounded-[2rem] p-6 text-sm text-white outline-none h-40 resize-none mb-6" placeholder="Frase de impacto para os clientes..."/>
                <button onClick={handleSaveHub} disabled={savingHub} className="w-full bg-[#FF1493] text-white py-6 rounded-2xl font-black uppercase text-xs">{savingHub ? <Loader2 className="animate-spin mx-auto"/> : "Salvar Bio"}</button>
            </div>
        )}
                        ...
                        <div className="flex flex-col justify-between">
                            <div className="space-y-4">
                                <div className="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/10">
                                    <input type="checkbox" checked={isPaidMedia} onChange={(e) => setIsPaidMedia(e.target.checked)} className="w-6 h-6 accent-[#FF1493]" />
                            ...
                            {!galleryPreviewUrl ? (
                                <label className="w-full bg-white/5 text-white py-6 rounded-2xl cursor-pointer hover:bg-white/10 border border-white/10 flex items-center justify-center gap-3 font-black uppercase text-[10px] mt-6">
                                    <Upload size={20}/> Escolher Arquivo (Máx 10MB)
                                    <input type="file" hidden accept="image/*" onChange={onChooseGalleryFile} />
                                </label>
                            ) : (
                                <div className="mt-6 space-y-3">
                                    <div className="relative aspect-video rounded-2xl overflow-hidden border-2 border-[#FF1493]">
                                    ...
                                    <button onClick={onPublishPhoto} disabled={uploading} className="w-full bg-[#FF1493] text-white py-6 rounded-2xl font-black uppercase text-[10px] shadow-xl flex items-center justify-center gap-3">
                                        {uploading ? <div className="flex flex-col items-center"><Loader2 className="animate-spin"/><span className="text-[8px] mt-1">SUBINDO ARQUIVO...</span></div> : <><Camera size={20}/> Publicar Agora</>}
                                    </button>
                                </div>
                            )}
                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {mediaList.map((item) => (
                        <div key={item.id} className="relative aspect-[3/4] rounded-3xl overflow-hidden group border border-white/5 bg-black">
                            ...
                            <button onClick={async () => { if(confirm("Apagar foto?")) { await fetch(`${supabaseUrl}/rest/v1/Media?id=eq.${item.id}`, { method: "DELETE", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}` } }); loadData(); } }} className="absolute inset-0 m-auto w-12 h-12 bg-red-500 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center"><Trash2 size={20}/></button>
                    ...
                    <div key={req.id} className="bg-black border border-white/5 p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between gap-8 shadow-2xl relative">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-4">
                                <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase ${req.status === 'pago' ? 'bg-blue-500' : req.status === 'aceito' ? 'bg-amber-500 text-black' : 'bg-emerald-500'}`}>{req.status}</span>
                        ...
                        <div className="min-w-[240px] flex flex-col justify-center gap-3">
                            {req.status === 'pago' && (
                                <>
                            ...
                            {req.status === 'aceito' && <input type="text" placeholder="Link do Drive + Enter" className="w-full bg-black border border-white/10 rounded-xl p-4 text-xs text-white" onKeyDown={async (e:any) => { if(e.key === 'Enter') { await fetch(`${supabaseUrl}/rest/v1/VideoRequests?id=eq.${req.id}`, { method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ drive_link: e.target.value, status: 'entregue' }) }); loadData(); } }} />}
                            {req.status === 'entregue' && <div className="text-emerald-500 text-[10px] font-black uppercase text-center flex items-center justify-center gap-2"><CheckCircle size={14}/> Vídeo Entregue</div>}
                        </div>
                    </div>
                )) : <div className="py-20 text-center text-white/10 italic">Nenhum pedido no momento.</div>}
            </div>
        )}

        {/* ROLETA (TOTALMENTE RESTAURADO) */}
        {activeTab === "roleta" && (
            <div className="space-y-6 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-black border border-white/10 p-8 rounded-[2.5rem] flex flex-col items-center text-center">
                        <div className="w-32 h-32 mb-4 bg-black/50 border border-white/10 rounded-full overflow-hidden">{(profilePreviewUrl || currentProfile) ? <img src={(profilePreviewUrl || currentProfile) as string} className="w-full h-full object-cover" /> : <User className="text-white/10" size={40} />}</div>
                        ...
                        {selectedProfileFile && <button onClick={async () => { setUploading(true); const fn=`profile_${modelId}_${Date.now()}.jpg`; await fetch(`${supabaseUrl}/storage/v1/object/assets/${fn}`, { method:"POST", headers:{apikey:supabaseKey!, Authorization:`Bearer ${supabaseKey}`, "Content-Type":selectedProfileFile.type}, body:selectedProfileFile }); const url=`${supabaseUrl}/storage/v1/object/public/assets/${fn}`; await fetch(`${supabaseUrl}/rest/v1/Configs?model_id=eq.${modelId}`, { method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ profile_url: url }) }); setCurrentProfile(url); setSelectedProfileFile(null); setUploading(false); alert("Salvo!"); }} className="w-full bg-[#FFD700] text-black py-3 rounded-xl text-[10px] font-black uppercase">Salvar Foto Vitrine</button>}
                    </div>
                    <div className="bg-black border border-white/10 p-8 rounded-[2.5rem] flex flex-col items-center text-center">
                        <div className="w-full h-32 mb-4 bg-black/50 border border-white/10 rounded-2xl overflow-hidden">{(bgPreviewUrl || currentBg) ? <img src={(bgPreviewUrl || currentBg) as string} className="w-full h-full object-cover" /> : <ImageIcon className="text-white/10" size={32} />}</div>
                ...
                <div className="bg-black border border-white/10 p-8 rounded-[3rem] shadow-2xl">
                    ...
                    <div className="grid gap-3">
                        {prizes.map((p, index) => {
                            ...
                            return (
                                <div key={p.id} className={`flex items-center justify-between p-5 rounded-3xl border ${isFake ? 'bg-indigo-500/5 border-indigo-500/20 opacity-80' : 'bg-black border-white/5'}`}>
                                    <div className="flex items-center gap-2">
                                        {!isFake && (<div className="flex flex-col gap-1 mr-2"><button onClick={() => movePrize(index, 'up')} className="p-1 rounded-md bg-white/5 hover:bg-[#FF1493]"><ChevronUp size={14}/></button><button onClick={() => movePrize(index, 'down')} className="p-1 rounded-md bg-white/5 hover:bg-[#FF1493]"><ChevronDown size={14}/></button></div>)}
                                    ...
                                    {!isFake && (<button onClick={() => setEditingPrize(p)} className="p-3 bg-white/5 rounded-xl text-white/40 hover:text-[#FF1493] transition-all"><Edit3 size={18}/></button>)}
                                </div>
                            );
                        })}
                    </div>
