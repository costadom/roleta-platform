...
import { 
  ...
  HelpCircle, ChevronUp, ChevronDown, User, Globe, Camera, Video, Send, Trash2, LayoutGrid, CheckCircle, Clock, AlertTriangle, Settings, Eye, EyeOff, Upload
} from "lucide-react";
...
function DashboardContent() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [resMod, resTrans, resPrizes, resConfig, resMedia, resVideos] = await Promise.all([
      ...
      setMediaList(resMedia); setVideoRequests(resVideos);
  ...
  // --- HANDLERS GALERIA (BOTOES DESUNIFICADOS) ---
  const onFileChangeGallery = (e: any) => {
  ...
  const publishGalleryPhoto = async () => {
    ...
    try {
        ...
        const upload = await fetch(`${supabaseUrl}/storage/v1/object/assets/${fileName}`, { method: "POST", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": selectedGalleryFile.type }, body: selectedGalleryFile });
        if (upload.ok) {
            ...
            setSelectedGalleryFile(null); setGalleryPreviewUrl(null); setRawPrice(""); setNewMediaCaption(""); loadData(); alert("Publicada!");
        }
    } catch (e) { alert("Erro ao publicar."); } finally { setUploading(false); }
  };

  // --- HANDLERS ROLETA (PRIZES) ---
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-right flex flex-col items-end">
            ...
            <p className="text-[#FFD700] text-[9px] font-bold uppercase tracking-[0.2em]">{modelSlug}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-8 bg-white/5 p-1.5 rounded-2xl border border-white/5 overflow-x-auto custom-scrollbar">
          ...
          <button onClick={() => setActiveTab("roleta")} className={`flex-1 min-w-[100px] py-3 rounded-xl text-[9px] font-black uppercase transition-all ${activeTab === "roleta" ? "bg-white/10 text-white" : "text-white/30"}`}>Roleta</button>
            ...
            <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-black border border-[#FFD700]/30 p-8 rounded-[2.5rem] shadow-2xl flex flex-col justify-center text-center">
                        <h2 className="text-xs font-black uppercase mb-2 text-[#FFD700]">Ganhos Acumulados</h2>
        ...
        {activeTab === "hub" && (
            <div className="max-w-3xl mx-auto bg-black border border-white/10 p-10 rounded-[3rem] shadow-2xl animate-in slide-in-from-bottom-4">
                ...
                <div className="space-y-6">
                    <label className="text-[10px] font-black uppercase text-white/40 mb-2 block ml-2">Sua Biografia / Frase de Impacto</label>
                ...
                <div className="bg-black border border-white/10 p-10 rounded-[3rem] mb-12 shadow-2xl">
                    <div className="grid md:grid-cols-2 gap-10">
                        <div className="space-y-4">
                            ...
                            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 text-[9px] text-white/40 uppercase font-black">⚠️ Fotos com nudez somente na categoria PAGA (Blur). Fotos grátis não podem ter nudez explícita.</div>
                        </div>
                        <div className="flex flex-col justify-between">
                            <div className="space-y-4">
                                ...
                                {isPaidMedia && (
                                    <div className="animate-in zoom-in">
                                        <label className="text-[10px] font-black text-white/40 mb-2 block ml-2">Preço (Mínimo R$ 10,00)</label>
                            ...
                            {!galleryPreviewUrl ? (
                                <label className="w-full bg-white/5 text-white py-6 rounded-2xl cursor-pointer hover:bg-white/10 border border-white/10 flex items-center justify-center gap-3 font-black uppercase text-[10px] mt-6 transition-all">
                                    ...
                                    <input type="file" hidden accept="image/*" onChange={onFileChangeGallery} />
                                </label>
                            ) : (
                                <div className="mt-6 space-y-3">
                                    ...
                                    {/* BOTÃO 2: PUBLICAR */}
                                    <button onClick={publishGalleryPhoto} disabled={uploading} className="w-full bg-[#FF1493] text-white py-6 rounded-2xl font-black uppercase text-[10px] shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {mediaList.map((item) => (
                        <div key={item.id} className="relative aspect-[3/4] rounded-3xl overflow-hidden group border border-white/5 bg-black">
                            ...
                            <button onClick={async () => { if(confirm("Apagar?")) { await fetch(`${supabaseUrl}/rest/v1/Media?id=eq.${item.id}`, { method: "DELETE", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}` } }); loadData(); } }} className="absolute inset-0 m-auto w-12 h-12 bg-red-500 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all"><Trash2 size={20}/></button>
        ...
        {activeTab === "video_requests" && (
            <div className="grid gap-6 animate-in fade-in">
                {videoRequests.length > 0 ? videoRequests.map((req) => (
                    <div key={req.id} className="bg-black border border-white/5 p-8 rounded-[2.5rem] flex flex-col md:flex-row justify-between gap-8 shadow-2xl relative">
                        ...
                        <div className="min-w-[240px] flex flex-col justify-center gap-3">
                            ...
                            {req.status === 'aceito' && <input type="text" placeholder="Link do Drive + Enter" className="w-full bg-black border border-white/10 rounded-xl p-4 text-xs text-white outline-none focus:border-emerald-500" onKeyDown={async (e:any) => { if(e.key === 'Enter') { await fetch(`${supabaseUrl}/rest/v1/VideoRequests?id=eq.${req.id}`, { method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ drive_link: e.target.value, status: 'entregue' }) }); loadData(); } }} />}
                ...
                )) : <div className="py-20 text-center text-white/10 italic font-black uppercase tracking-widest border border-dashed border-white/5 rounded-[3rem]">Nenhum pedido no momento.</div>}
            </div>
        )}

        {/* --- ROLETA (RESTAURADA COMPLETA) --- */}
        {activeTab === "roleta" && (
            <div className="space-y-6 animate-in fade-in">
                <div className="bg-white/5 border border-white/10 p-6 rounded-3xl flex items-center justify-between">
                    <div><h3 className="text-[11px] font-black uppercase text-[#FFD700]">Nome e Visibilidade</h3></div>
                    <div className="flex items-center gap-6">
                        <input type="text" value={modelName} onChange={(e) => setModelName(e.target.value)} onBlur={async () => { await fetch(`${supabaseUrl}/rest/v1/Configs?model_id=eq.${modelId}`, { method: "PATCH", headers: { apikey: supabaseKey!, Authorization: `Bearer ${supabaseKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model_name: modelName }) }); }} className="bg-black border border-white/10 px-4 py-2 rounded-xl text-[10px] font-black uppercase text-[#FFD700] outline-none w-48" placeholder="Nome da Musa" />
          ...
          <form onSubmit={async (e) => { 
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white/5 p-4 rounded-2xl space-y-3">
                    <p className="text-[10px] font-black uppercase text-[#FFD700]">Entrega do Conteúdo</p>
