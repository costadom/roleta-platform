  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [resGlob, resModel, resNotif, resTrans, resPrizes, resConfig, resMyHist, resRankHist, resAllMod] = await Promise.all([
        fetch(`${supabaseUrl}/rest/v1/GlobalSettings?id=eq.main&select=*`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Models?id=eq.${modelId}&select=balance,last_withdrawal,pix_key_1,pix_key_2,terms_accepted`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Withdrawals?model_id=eq.${modelId}&status=eq.pago&is_read=eq.false`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Transactions?model_id=eq.${modelId}&created_at=gte.${sixMonthsAgo.toISOString()}&select=model_cut`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Prize?model_id=eq.${modelId}&select=*`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Configs?model_id=eq.${modelId}&select=*`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/SpinHistory?model_id=eq.${modelId}&select=*&order=created_at.desc&limit=50`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/SpinHistory?select=model_id&limit=2000`, fetchOpts),
        fetch(`${supabaseUrl}/rest/v1/Models?select=id,slug`, fetchOpts)
      ]);
