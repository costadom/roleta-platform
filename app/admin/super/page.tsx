...
export default function SuperAdmin() {
  ...
  // 🔥 ESTADO DAS MENSAGENS DE WHATSAPP 🔥
  ...
  if (!isLogged) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6"><div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl">
      <ShieldCheck size={40} className="text-[#FF1493] mx-auto mb-6"/><h1 className="text-xl font-black text-white mb-8 uppercase tracking-widest">Master Access</h1>
      <form onSubmit={handleLogin} className="space-y-4">
        <input type="password" placeholder="SENHA MESTRA" className="w-full bg-black border border-white/10 p-5 rounded-2xl text-xs text-white outline-none" value={adminPass} onChange={e => setAdminPass(e.target.value)} />
        <button type="submit" className="w-full bg-[#FF1493] text-white py-5 rounded-2xl font-black uppercase">Entrar no Painel</button>
      </form>
    </div></div>
  );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 gap-6 mb-12">
            {videoRequests.length > 0 && (
              <div className="bg-blue-500/10 border border-blue-500/30 p-6 rounded-[2.5rem] shadow-xl animate-in slide-in-from-top-4">
                <h2 className="text-xs font-black uppercase text-blue-400 mb-4 flex items-center gap-2 tracking-widest"><Video size={16}/> {videoRequests.length} Vídeos Solicitados (Pagar Musas)</h2>
            ...
            {abandoned.length > 0 && (
              <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-[2.5rem] shadow-xl animate-in slide-in-from-top-4">
                  ...
                  {abandoned.map(cart => (
                    <div key={cart.id} className="bg-black border border-red-500/20 p-5 rounded-3xl flex flex-col justify-between">
                      <div className="mb-4">
                        ...
                        <p className="text-[10px] text-red-400 font-bold mb-1 uppercase">Tentou R$ {Number(cart.amount).toFixed(2)} na {cart.model_name}</p>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => window.open(`https://wa.me/${(cart.player_phone || cart.whatsapp)?.replace(/\D/g, '')}?text=${encodeURIComponent(`Oii! Vi que seu PIX na plataforma da ${cart.model_name} não concluiu. Quer ajuda?`)}`)} className="flex-1 bg-emerald-500 text-black py-3 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-1 hover:scale-105 transition-all"><MessageCircle size={14}/> Chamar Fã</button>
                        <button onClick={() => handleAction('ignoreAbandoned', { id: cart.id })} className="p-3 bg-white/5 border border-white/10 rounded-xl text-white/30 hover:text-red-500"><X size={14}/></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {applications.length > 0 && (
              <div className="bg-indigo-500/10 border border-indigo-500/30 p-6 rounded-[2.5rem] shadow-xl animate-in slide-in-from-top-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {applications.map(app => (
                    <div key={app.id} onClick={() => setSelectedApp(app)} className="bg-black border border-indigo-500/20 p-5 rounded-3xl flex flex-col justify-between cursor-pointer hover:border-indigo-400 transition-all group">
                      <div className="mb-4"><p className="text-[12px] text-white uppercase font-black">{app.full_name}</p><p className="text-[10px] text-indigo-400 font-bold uppercase">@{app.nickname}</p></div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-[#FF1493]/10 border border-[#FF1493]/30 p-8 rounded-[2.5rem] relative overflow-hidden"><p className="text-[10px] font-black text-[#FF1493] uppercase mb-1 relative z-10">Faturamento Bruto</p><h3 className="text-4xl font-black text-white relative z-10">{financialData.sales.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h3></div>
            <div className="bg-emerald-500/10 border border-emerald-500/30 p-8 rounded-[2.5rem] relative overflow-hidden"><p className="text-[10px] font-black text-emerald-500 uppercase mb-1 relative z-10">Lucro Savanah (30%)</p><h3 className="text-4xl font-black text-white relative z-10">{financialData.plat.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h3></div>
                ...
                <div key={m.id} className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2.5rem] hover:border-[#FF1493]/30 transition-all shadow-xl group relative overflow-hidden">
                  <div className="flex justify-between items-start mb-4 relative z-10">
                    ...
                    <div className="flex gap-2">
                       ...
                       <button onClick={() => { if(confirm(`Excluir @${m.slug}?`)) handleAction('deleteModel', {id: m.id}); }} className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button>
                    </div>
                  </div>
              ...
              {withdrawals.filter(w => w.status === 'pendente').map(w => (
                  <div key={w.id} className="bg-amber-500/5 border border-amber-500/20 p-5 rounded-3xl animate-in zoom-in-95">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <button onClick={() => handleAction('approveWithdrawal', { id: w.id })} className="bg-amber-500 text-black px-5 py-2.5 rounded-xl text-[9px] font-black uppercase hover:scale-105 transition-all shadow-lg shadow-amber-500/20">Pagar</button>
                    </div>
      ...
      {selectedApp && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0a0a] border border-indigo-500/30 p-8 rounded-[3rem] w-full max-w-lg shadow-2xl relative overflow-y-auto max-h-[90vh]">
            ...
            <button onClick={() => handleAction('rejectApplication', { id: selectedApp.id })} className="w-full mt-4 py-3 text-[9px] font-black uppercase text-red-500 hover:bg-red-500/10 rounded-xl transition-all">Rejeitar Candidatura</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0a0a0a] border border-white/10 p-8 rounded-[3rem] w-full max-w-md relative shadow-2xl">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAction('createModel', newModel); setShowModal(false); }} className="space-y-4">
              <input type="text" required placeholder="Nickname/Slug" value={newModel.slug} onChange={e => setNewModel({ ...newModel, slug: e.target.value })} className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-white text-sm outline-none" />
              <input type="email" required placeholder="E-mail de Login" value={newModel.email} onChange={e => setNewModel({ ...newModel, email: e.target.value })} className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-white text-sm outline-none" />
              <input type="text" required placeholder="Senha de Acesso" value={newModel.password} onChange={e => setNewModel({ ...newModel, password: e.target.value })} className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-white text-sm outline-none" />
              <button type="submit" disabled={loading} className="w-full bg-[#FF1493] text-white py-5 rounded-2xl font-black uppercase shadow-lg shadow-[#FF1493]/20">Gerar Franquia Agora</button>
            </form>
          </div>
        </div>
      )}
