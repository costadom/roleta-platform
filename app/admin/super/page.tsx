...
export default function SuperAdmin() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Busca plana para evitar erros 400
  ...
  // FUNÇÃO CORRIGIDA PARA O X DO PIX ABANDONADO
  const handleIgnoreCart = async (id: string) => {
      ...
      fetchData(); // Recarrega a tela para a lixeira sumir
  };

  // DECLARAÇÃO DA FUNÇÃO DO COMUNICADO GLOBAL (Que estava causando o ReferenceError)
  const handleSaveGlobal = async () => {
    ...
    try {
        await fetch(`${supabaseUrl}/rest/v1/GlobalSettings?id=eq.main`, { 
            ...
            body: JSON.stringify({ announcement_msg: globalMsg }) 
        });
    ...
    <div className="min-h-screen bg-[#050505] text-white p-4 sm:p-10 font-sans pb-24">
      <div className="max-w-7xl mx-auto">
                  ...
                  <div key={cart.id} className="bg-black border border-red-500/20 p-5 rounded-3xl flex flex-col justify-between">
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase flex items-center justify-center gap-1 ${phone ? 'bg-emerald-500 text-black hover:bg-emerald-400' : 'bg-white/5 text-white/20'}`}
                      >
                        <MessageCircle size={14}/> {phone ? 'Chamar no Zap' : 'Sem Telefone'}
                      </button>
                      <button onClick={() => handleIgnoreCart(cart.id)} className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white/30 hover:text-red-500 hover:bg-red-500/10 transition-all"><X size={14}/></button>
                    </div>
                  </div>
                )
              })}
                        ...
                        return (
                            <div key={w.id} className="bg-black/50 border border-white/5 p-6 rounded-3xl flex items-center justify-between">
                                ...
                                <button onClick={async () => {
                                    ...
                                    if(m?.whatsapp) window.open(`https://wa.me/${m.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Oii! Seu PIX de R$ ${Number(w.amount).toFixed(2)} foi enviado! 💸`)}`, '_blank');
                                    fetchData();
        ...
        <div className="mt-12 bg-[#0a0a0a] border border-white/5 p-10 rounded-[3rem] relative overflow-hidden">
          ...
          <button onClick={handleSaveGlobal} disabled={savingGlobal} className="w-full bg-white text-black py-4 rounded-xl text-[9px] font-black uppercase shadow-lg transition-all hover:bg-[#FF1493] hover:text-white">{savingGlobal ? "Salvando..." : "ENVIAR COMUNICADO"}</button>
        </div>
      </div>
