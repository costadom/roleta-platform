  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // 1. Busca Plana dos Dados Essenciais (Sem Aninhamento de PostgREST)
        const [playersRes, modelsRes, configsRes] = await Promise.all([
            fetch(`${supabaseUrl}/rest/v1/Players?whatsapp=eq.${phone}&select=*`, { headers }),
        ...
        if (playerIds.length > 0) {
            ...
            // 2. Busca Vídeos (Simples e Seguro)
            ...
            // 3. Busca Fotos Desbloqueadas (Simples e Seguro)
            const uRes = await fetch(`${supabaseUrl}/rest/v1/UnlockedMedia?player_id=in.(${idsString})&select=*`, { headers });
            if (uRes.ok) {
                const uData = await uRes.json();
                const mediaIds = uData.map((u:any) => u.media_id).filter(Boolean);
      ...
      {viewingMedia && (
          <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-2xl flex flex-col items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
             ...
             <div className="flex flex-col items-center gap-4 text-center max-w-md w-full">
                ...
                <div>
                    <p className="text-[10px] font-black text-[#D946EF] uppercase tracking-widest mb-1">{viewingMedia.modelInfo?.model_name || viewingMedia.modelInfo?.slug}</p>
