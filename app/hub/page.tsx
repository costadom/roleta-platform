...
export default function PlayerPersonalHub() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#050505] text-white font-sans pb-24 relative overflow-x-hidden">
      ...
      {/* MODAIS (LOJA, PIX, CHAT, PRESENTE, VIEWING) */}
      {showShopModal && (
              ...
              <div className="relative w-full max-w-lg h-[85vh] sm:h-[700px] bg-[#0a0a0a] border border-white/10 sm:rounded-[3rem] rounded-t-[3rem] flex flex-col shadow-2xl overflow-hidden z-10 animate-in slide-in-from-bottom-10 duration-300">
                  ...
                  <div className="p-5 bg-[#0a0a0a]/90 backdrop-blur-xl border-t border-white/5 z-20 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
                      <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full p-1.5 pl-5 focus-within:border-[#D946EF]/50 focus-within:bg-white/10 transition-all shadow-inner">
                          ...
                          <button onClick={handleSendMessage} disabled={!newMessage.trim()} className="w-12 h-12 rounded-full bg-gradient-to-r from-[#D946EF] to-[#a832b8] text-white flex items-center justify-center shadow-[0_0_15px_rgba(217,70,239,0.4)] hover:scale-105 active:scale-95 transition-all shrink-0 disabled:opacity-50"><Send size={18} className="-ml-0.5" /></button>
                      </div>
                  </div>
              </div>
          </div>
              ...
              <div className="bg-[#0a0a0a] border border-amber-500/30 p-8 rounded-[3rem] w-full max-w-md shadow-2xl relative">
                  ...
                  <div className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                          {[ { name: "Beijo 💋", value: 10 }, { name: "Drink 🍸", value: 30 }, { name: "Lingerie 👙", value: 50 }, { name: "VIP 👑", value: 150 } ].map((gift) => (
                              <button key={gift.value} onClick={() => setSelectedGift(gift.value)} className={`p-4 rounded-3xl border flex flex-col items-center justify-center text-center transition-all ${selectedGift === gift.value ? 'bg-amber-500/20 border-amber-500 text-amber-400' : 'bg-black/50 border-white/10 text-white/50'}`}>
                                  <span className="text-[10px] font-black uppercase mb-2">{gift.name}</span>
                                  <span className="text-lg font-black">R$ {gift.value}</span>
                              </button>
                          ))}
                      </div>
                      <input type="text" value={giftMessage} onChange={(e) => setGiftMessage(e.target.value)} placeholder="Deixe uma mensagem (opcional)..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-sm text-white outline-none focus:border-amber-500 transition-all" />
                      <button onClick={() => generatePix(selectedGift, 'gift', undefined, giftMessage)} disabled={generatingPix} className="w-full bg-amber-500 text-black py-6 rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">
      ...
      {viewingMedia && (
          <div className="fixed inset-0 z-[700] bg-black/95 backdrop-blur-2xl flex flex-col items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
             <button onClick={() => setViewingMedia(null)} className="absolute top-6 right-6 text-white/50 hover:text-white bg-white/10 p-3 rounded-full z-[310]"><X size={20}/></button>
             <div className="relative max-w-3xl w-full h-[65vh] flex items-center justify-center mb-8">
             ...
             <div className="flex flex-col items-center gap-5 text-center max-w-md w-full bg-black/50 p-6 rounded-[2.5rem] border border-white/5 backdrop-blur-md">
                <button onClick={() => setLiked(!liked)} className={`p-4 rounded-full transition-all border ${liked ? 'bg-red-500 text-white border-red-400' : 'bg-white/5 text-white/50'}`}>
