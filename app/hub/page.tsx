  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // 1. BUSCA DADOS PLANOS (ESPIÃO ATIVADO)
        ...
        if (!pRes.ok) { console.error("🕵️ ESPIÃO: Erro na tabela Players", await pRes.text()); return; }
        ...
        // 2. FILTRO DE SEGURANÇA ANTI-ERRO 400
        const ids = pData.map((p:any) => p.id).filter((id:any) => id && id.length > 20);
        console.log("🕵️ ESPIÃO: IDs filtrados para busca:", ids);

        if (ids.length > 0) {
            const queryIds = `(${ids.join(',')})`;

            // BUSCA VÍDEOS
            const vRes = await fetch(`${supabaseUrl}/rest/v1/VideoRequests?player_id=in.${queryIds}&select=*`, { headers });
            if (vRes.ok) {
                const vD = await vRes.json();
                setVideoOrders(vD.map((v:any) => ({ ...v, modelInfo: getInfo(v.model_id) })).filter((v:any) => v.status !== 'pendente'));
            } else { console.warn("🕵️ ESPIÃO: Falha nos vídeos", await vRes.text()); }

            // BUSCA FOTOS
            const uRes = await fetch(`${supabaseUrl}/rest/v1/UnlockedMedia?player_id=in.${queryIds}&select=*`, { headers });
            if (uRes.ok) {
                const uD = await uRes.json();
                const mIds = uD.map((u:any) => u.media_id).filter(Boolean);
                if (mIds.length > 0) {
                    const mediaRes = await fetch(`${supabaseUrl}/rest/v1/Media?id=in.(${mIds.join(',')})&select=*`, { headers });
                    const mediaData = await mediaRes.json();
                    setUnlockedGallery(uD.map((u:any) => {
                        const mObj = mediaData.find((mx:any) => mx.id === u.media_id);
                        return mObj ? { ...u, Media: { ...mObj, modelInfo: getInfo(mObj.model_id) } } : null;
                    }).filter(Boolean));
                }
            } else { console.warn("🕵️ ESPIÃO: Falha nas fotos", await uRes.text()); }
        }
      } catch (e) { console.error("🕵️ ESPIÃO: Erro Crítico", e); } finally { setInitialLoading(false); }
    }
    loadData();
  }, [router]);

  if (initialLoading) return <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white"><Loader2 className="animate-spin text-[#D946EF] mb-6" size={50} /><h2 className="text-xl font-black uppercase italic animate-pulse tracking-tighter">Acessando Universo Privado...</h2></div>;

  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-24">
      <header className="fixed top-0 left-0 w-full h-20 bg-black/70 backdrop-blur-xl border-b border-white/5 z-[100] px-6 flex items-center justify-between">
          ...
          <div className="text-center"><h1 className="text-lg font-black uppercase italic text-[#D946EF] tracking-tighter">MEU <span className="text-white">HUB VIP</span></h1><p className="text-[9px] text-white/30 uppercase font-black tracking-widest">{playerPhone}</p></div>
                ...
                {associations.length > 0 ? associations.map((assoc: any) => (
                    <div key={assoc.id} className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2rem] shadow-xl flex flex-col justify-between gap-5 relative overflow-hidden group hover:border-[#D946EF]/30 transition-all min-h-[140px]">
                        <div className="flex items-center gap-4 relative z-10">
                            <div className="w-14 h-14 rounded-full bg-black border-2 border-[#D946EF] overflow-hidden shrink-0 flex items-center justify-center">
        ...
        <section className="mb-16">
            ...
            <div className="grid gap-6">
                {videoOrders.length > 0 ? videoOrders.map((req) => (
                    <div key={req.id} className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between gap-6 shadow-xl relative overflow-hidden">
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-4 text-[10px] font-bold uppercase tracking-widest">
                                <span className={`px-3 py-1 rounded-full ${req.status === 'pago' ? 'bg-blue-500/20 text-blue-400' : 'bg-emerald-500/20 text-emerald-400'}`}>{req.status}</span>
                                <span>Para: {req.modelInfo?.model_name} • {req.duration} Min • R$ {req.price?.toFixed(2)}</span>
                            </div>
