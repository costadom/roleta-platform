...
import { 
  ...
  LayoutGrid, X, Send, Video, Clock, CheckCircle, Info 
} from "lucide-react";
...
export default function ModelProfile() {
  ...
  const handleRequestVideo = async () => {
    ...
    try {
      ...
      alert("Pedido enviado! Acompanhe no seu painel.");
      ...
      <div className="max-w-7xl mx-auto p-8">
        <div className="bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] p-8 mb-12 flex flex-col md:flex-row gap-8 items-center shadow-2xl relative overflow-hidden">
            <div className="flex-1">
                ...
                <p className="text-white/60 text-sm italic mb-6">Peça um vídeo personalizado. Entrega em 2 dias úteis ou seu dinheiro de volta via suporte.</p>
                <div className="flex gap-4"><div className="bg-white/5 border border-white/10 px-4 py-2 rounded-xl flex items-center gap-2"><Clock size={14} className="text-[#D946EF]"/><span className="text-[10px] font-black uppercase">2 Dias</span></div><div className="bg-white/5 border border-white/10 px-4 py-2 rounded-xl flex items-center gap-2"><CheckCircle size={14} className="text-emerald-500"/><span className="text-[10px] font-black uppercase">Garantia</span></div></div>
            </div>
            <button onClick={() => setShowVideoModal(true)} className="w-full md:w-auto bg-white text-black px-10 py-6 rounded-3xl font-black uppercase text-xs hover:bg-[#D946EF] hover:text-white transition-all">Encomendar Vídeo</button>
        </div>
            ...
            return (
              <div key={item.id} className="flex flex-col gap-4 group">
                <div className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden border border-white/5 bg-[#0a0a0a] cursor-pointer shadow-2xl"
                    onClick={() => { if(!isLoggedIn) return setShowAuth(true); if(!isUnlocked) alert("Pagamento Foto R$ " + item.price); }}>
                    ...
                    {!isUnlocked && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                            ...
                            <div className="bg-[#D946EF] px-5 py-2 rounded-full text-[10px] font-black uppercase shadow-xl">Liberar R$ {item.price}</div>
                        </div>
                    )}
                    {item.price === 0 && <div className="absolute top-5 left-5 bg-emerald-500 text-[8px] font-black uppercase px-3 py-1.5 rounded-xl">Livre</div>}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl">
            <div className="bg-[#0a0a0a] border border-white/10 p-10 rounded-[3.5rem] w-full max-w-lg shadow-2xl relative">
                ...
                <div className="grid grid-cols-3 gap-2 mb-8">
                    {[3, 5, 10].map(d => (
                        <button key={d} onClick={() => setSelectedDuration(d as any)} className={`py-4 rounded-2xl border text-[10px] font-black transition-all ${selectedDuration === d ? 'bg-[#D946EF] border-[#D946EF] text-white shadow-lg' : 'bg-black border-white/10 text-white/40'}`}>
                            {d} MIN<br/>R$ {pricing[d as keyof typeof pricing]}
                        </button>
                    ))}
                </div>
                <textarea value={videoDesc} onChange={(e) => setVideoDesc(e.target.value)} className="w-full bg-black border border-white/10 rounded-[2rem] p-6 text-sm text-white outline-none focus:border-[#D946EF] h-40 resize-none mb-8" placeholder="O que você quer ver no vídeo?"/>
                <button onClick={handleRequestVideo} disabled={requesting} className="w-full bg-[#D946EF] text-white py-6 rounded-2xl font-black uppercase text-xs shadow-2xl flex items-center justify-center gap-3 hover:bg-[#f062ff] transition-all">
                    {requesting ? <Loader2 className="animate-spin" size={20}/> : <Send size={18}/>} Pagar R$ {pricing[selectedDuration]}
                </button>
            </div>
        </div>
      )}
